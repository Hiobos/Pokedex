...
class GraphicInterface:
    def __init__(self):
        ...
        self.canvas = Canvas(self.window, width=500, height=500, bg='lightyellow')
        self.images = []
        self.column = 0
        self.row = 0
    ...
    def add_pokemon(self, pokemon_id):
        ...
        for i in range(0, 151):
            ...
            pokemon_img = Label(frame, image=photo)
            pokemon_img.grid(row=0, column=self.column)
            #pokemon name
            pokemon_name = Label(frame, text=pokemon_list[i].capitalize())
            pokemon_name.grid(row=1, column=self.column)
            #pokemon type
            pokemon_type = Label(frame, text=pokemon.types[0].type.name)
